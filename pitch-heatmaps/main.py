...
CONTACT_DESC = ["hit_into_play", "foul"]

if __name__ == '__main__':
    df = pd.read_csv("statcast_data_2025.csv")
    # df_out = df[~df["zone"].between(1, 9, inclusive="both")].copy()
    metrics = outside_zone_df(df, SWUNG_DESC, CONTACT_DESC)

    player = "Vladimir Guerrero Jr."
    draw_strike_zone(df, player, SWUNG_DESC, CONTACT_DESC)
    #
